...
namespace DAL
{
    ...
    string sql = "select count(1) from activity where activityVerify='审核通过'";
        ...
        public IEnumerable<Model.Activity> GetNew()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from activity where activityVerify='审核通过' and endTime>=now() order by end   Time desc limit 2";
        ...
        public int UpdateImg(Model.Activity active)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update activity set activityname=@activityName where activityid=@activityId";
